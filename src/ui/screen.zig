...
const master_password_popup = @import("master_password_popup.zig");
const theme = @import("theme.zig");
const unlock_screen = @import("unlock_screen.zig");
...
fn iconButton(
    ...
    const options = theme.buttonOptions(opts, palette, style);
...
fn iconTextButton(
    ...
    var bw: dvui.ButtonWidget = undefined;
    const options = theme.buttonOptions(opts, palette, style).override(.{
...
fn topBarSettingsButton(bytes: []const u8, name: []const u8, opts: dvui.Options, palette: theme.Palette, id_base: usize) IconButtonInfo {
    var bw: dvui.ButtonWidget = undefined;
    const options = theme.buttonOptions(opts, palette, .{ .variant = .ghost, .font_size = theme.font_sizes.tab });
    bw.init(@src(), .{ .draw_focus = false }, options);
    bw.processEvents();
    bw.drawBackground();
...
fn connectionRow(app: *App, item: profile.ConnectionProfile, row_idx: usize, palette: theme.Palette) void {
    ...
    const profile_button = iconTextButton(@src(), server_icon_bytes, "server.png", label, .{
    ...
    if (profile_button.hovered or row_hovered) {
        if (iconButton(@src(), settings_icon_bytes, "settings.png", .{
            .gravity_x = 1,
            .gravity_y = 0.5,
